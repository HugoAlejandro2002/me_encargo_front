import { Form, Input, InputNumber, Button, Space, Card, Typography, Divider, Table } from 'antd';
...
const CashReconciliationForm = ({ 
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "total",
      render: (denomination: string, _: any, type: 'coins' | 'bills') => 
        `Bs. ${(type === 'coins' ? coinTotals[denomination] : billTotals[denomination]) || 0}`,
    },
  ];
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      cash_delivery: 0,
      observations: '',
      coins: coinDenominations.reduce((acc, denom) => ({ ...acc, [denom]: 0 }), {}),
      bills: billDenominations.reduce((acc, denom) => ({ ...acc, [denom]: 0 }), {}),
  ...
  const handleSubmit = async (values: ReconciliationFormData) => {
    try {
      const totalCoins = Object.entries(coinTotals).reduce((sum, [_, total]) => sum + total, 0);
      const totalBills = Object.entries(billTotals).reduce((sum, [_, total]) => sum + total, 0);
      const totalCash = totalCoins + totalBills;
      
      const expected_balance = values.opening_balance + values.cash_sales - 
        values.cash_expenses - values.cash_delivery;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <InputNumber
              prefix="Bs."
              className="w-full"
              value={Object.values(coinTotals).reduce((a, b) => a + b, 0) + 
